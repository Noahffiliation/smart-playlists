...
def test_get_followed_artists(mock_sp, mock_logger):
    ...
    mock_sp.next.return_value = {"artists": {"items": [{"name": "Artist 2"}], "next": None}}
...
def test_get_saved_tracks(mock_sp, mock_logger):
    ...
    mock_sp.next.return_value = {"items": [{"track": {"id": "t2"}}], "next": None}
...
def test_get_all_library_tracks(mock_sp, mock_logger):
    with (
        patch("new_releases.get_saved_tracks") as mock_saved,
...
def test_get_album_tracks(mock_sp):
    mock_sp.album_tracks.return_value = {"items": [{"id": "t1"}], "next": "url"}
    mock_sp.next.return_value = {"items": [{"id": "t2"}], "next": None}
...
def test_main_with_new_tracks(mock_sp, mock_logger):
    with (
        patch(
            "new_releases.get_followed_artists",
            return_value=[{"name": "A1", "id": "aid1"}, {"name": "A2", "id": "aid2"}],
        ),
        ...
        patch(
            "new_releases.get_artist_new_releases",
            side_effect=[
                [{"id": "alb1", "name": "Album 1", "release_date": "2026-01-28"}],
                [
                    {"id": "alb1", "name": "Album 1", "release_date": "2026-01-28"}
                ],  # Duplicate album skipped
            ],
        ),
...
def test_main_no_new_tracks(mock_sp, mock_logger):
    with (
        ...
        patch(
            "new_releases.get_artist_new_releases",
            return_value=[{"id": "alb_id", "name": "Alb", "release_date": "2026-01-28"}],
        ),
        patch("new_releases.get_album_tracks", return_value=["t1"]),
    ):
...
def test_main_exception(mock_sp, mock_logger):
    with (
        patch("new_releases.get_followed_artists", side_effect=RuntimeError("Fatal error")),
