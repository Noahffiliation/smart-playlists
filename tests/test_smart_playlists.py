...
def test_normalize_track_title():
    assert smart_playlists.normalize_track_title("Bohemian Rhapsody (2011 Remaster)") == "bohemian rhapsody"
...
def test_get_liked_songs(mock_spotify):
    ...
    items_500 = [{'track': {'uri': f'u_{i}'}, 'added_at': '2026-01-28T13:00:00Z'} for i in range(500)]
...
def test_get_lastfm_track_playcount(mock_lastfm):
    ...
    count = smart_playlists.get_lastfm_track_playcount('Artist', 'Track', lastfm_network=mock_lastfm)
    assert count == 10

    mock_track.get_userplaycount.return_value = None
    assert smart_playlists.get_lastfm_track_playcount('Artist', 'Track', lastfm_network=mock_lastfm) == 0
...
def test_update_recent_tracks_playlist(mock_spotify):
    with patch('smart_playlists.get_all_spotify_library_tracks') as mock_library, \
        ...
        smart_playlists.update_recent_tracks_playlist(mock_library.return_value, 'Target', client=mock_spotify)
...
def test_match_spotify_with_lastfm(mock_lastfm):
    ...
    spotify_tracks = {
        f'uri_{i}': {'name': f'Name{i} (Remastered)', 'artist': f'Artist{i}'}
        for i in range(105)
    }
    ...
    with patch('smart_playlists.get_all_lastfm_playcounts') as mock_bulk:
        ...
        result = smart_playlists.match_spotify_with_lastfm(spotify_tracks, lastfm_network=mock_lastfm)
...
def test_get_all_lastfm_playcounts(mock_lastfm):
    ...
    result = smart_playlists.get_all_lastfm_playcounts(lastfm_network=mock_lastfm, username='test_user')
    ...
    assert smart_playlists.get_all_lastfm_playcounts(lastfm_network=mock_lastfm, username='test_user') == {}
...
def test_update_playcount_playlists(mock_spotify):
    ...
    with patch('smart_playlists.match_spotify_with_lastfm', return_value=tracks), \
         patch('smart_playlists.create_or_update_playlist') as mock_create_update:
...
def test_main():
    with patch('smart_playlists.get_all_spotify_library_tracks', return_value={}) as mock_lib, \
        ...
        mock_playcount.assert_called_with({}, 'Top 25', 'Bottom 25', client=None, lastfm_network=None)
