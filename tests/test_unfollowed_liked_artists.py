...
def test_get_followed_artist_ids():
    ...
    mock_sp.current_user_followed_artists.return_value = {
        "artists": {"items": [{"id": "a1", "name": "Artist 1"}], "next": "url"}
    }
    mock_sp.next.return_value = {
        "artists": {"items": [{"id": "a2", "name": "Artist 2"}], "next": None}
    }
...
def test_find_unfollowed_liked_artists():
    ...
    results = unfollowed_liked_artists.find_unfollowed_liked_artists(
        counts, names, followed_ids, min_liked=10
    )
    ...
    results_5 = unfollowed_liked_artists.find_unfollowed_liked_artists(
        counts, names, followed_ids, min_liked=5
    )
...
def test_main():
    ...
    with (
        patch("utils.unfollowed_liked_artists.get_followed_artist_ids", return_value={"a1"}),
        patch(
            "utils.unfollowed_liked_artists.count_liked_songs_by_artist",
            return_value=({"a2": 15}, {"a2": "Artist 2"}),
        ),
    ):
        # Matches found
        res_matched = unfollowed_liked_artists.main(
            ["--min", "10"], client=mock_sp, custom_logger=mock_logger
        )
        assert len(res_matched) == 1

        # No matches found
        res_unmatched = unfollowed_liked_artists.main(
            ["--min", "20"], client=mock_sp, custom_logger=mock_logger
        )
